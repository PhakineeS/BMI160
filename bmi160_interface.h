...
// int8_t i2c_get_data(uint8_t i2cbus);
class BMI160{
    public:
        BMI160();
        bmi160_dev initialize(int8_t &rslt);
        void set_sensor_settings(struct bmi160_dev *dev, int mode, int8_t &rslt);
        void read_sensor_data(struct bmi160_dev *dev, int8_t &rslt, struct bmi160_sensor_data &accel, struct bmi160_sensor_data &gyro);
};
